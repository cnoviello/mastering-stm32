...
#define APP_START_ADDRESS   0x08003000 /* In STM32F334R8 this corresponds with the start
                                        address of Page 6 */

#define SRAM_SIZE           12*1024     // STM32F334R8 has 12KB of RAM
#define SRAM_END            (SRAM_BASE + SRAM_SIZE)
#define FLASH_TOTAL_PAGES   31
...
void cmdErase(uint8_t *pucData) {
  ...
  if (ulCrc == HAL_CRC_Calculate(&hcrc, pulCmd, 2) &&
      (pucData[1] > 0 && (pucData[1] < FLASH_TOTAL_PAGES - 5 || pucData[1] == 0xFF))) {
    ...
    eraseInfo.NbPages = pucData[1] == 0xFF ? FLASH_TOTAL_PAGES - 5 : pucData[1];
