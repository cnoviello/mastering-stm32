...
/*****************************************************/
    ...
    {
      *(buf+blocklen+1) = 0; // Insert '/0' for indicates the 'End of String' (null terminated)
    }
  ...
  HTTPSock_Status[get_seqnum].file_offset += send_len;
	...
	if(HTTPSock_Status[get_seqnum].file_len == HTTPSock_Status[get_seqnum].file_offset)
	{
		...
		flag_datasend_end = 0;
#if defined(_USE_SDCARD_) && !defined(OS_USE_SEMIHOSTING)
		f_close(&HTTPSock_Status[get_seqnum].fs);
#elif defined(OS_USE_SEMIHOSTING)
		fclose(HTTPSock_Status[get_seqnum].fs);
#endif
	}
	else
	{
