...
#define APP_START_ADDRESS   0x08002000 /* In STM32L152RE this corresponds with the start
                                        address of Sector 2 */

#define SRAM_SIZE           80*1024     // STM32L152RE has 80KB of RAM
#define SRAM_END            (SRAM_BASE + SRAM_SIZE)
#define FLASH_TOTAL_PAGES   2047

#define ENABLE_BOOTLOADER_PROTECTION 0
#define SECTORS_TO_PROTECT (OB_WRP1_PAGES0TO15 | OB_WRP1_PAGES16TO31)
/* Private variables ---------------------------------------------------------*/
...
void cmdErase(uint8_t *pucData) {
  ...
  if (ulCrc == HAL_CRC_Calculate(&hcrc, pulCmd, 2) &&
      (pucData[1] > 0 && (pucData[1] < FLASH_TOTAL_PAGES - 128 || pucData[1] == 0xFF))) {
    ...
    eraseInfo.NbPages = pucData[1] == 0xFF ? FLASH_TOTAL_PAGES - 128   : pucData[1];
...
void CHECK_AND_SET_FLASH_PROTECTION(void) {
  ...
  if ((obConfig.WRPSector0To31 & SECTORS_TO_PROTECT) == SECTORS_TO_PROTECT) {
    ...
    obConfig.WRPSector0To31 = SECTORS_TO_PROTECT; //Enables WP on first sectors
