...
void vPortSuppressTicksAndSleep(TickType_t xExpectedIdleTime) {
  ...
  /* Stop LPTIM1 momentarily.  The time LPTIM1 is stopped for is not accounted for
   ...
   anyway. */
  lptim1CNT = HAL_LPTIM_ReadCounter(&hlptim1);
  ...
  if (eSleepAction == eAbortSleep) {
    /* Restart tick. */
    HAL_LPTIM_Counter_Start(&hlptim1, ulPeriodValueForOneTick - lptim1CNT);
...
//      __HAL_LPTIM_AUTORELOAD_SET(&hlptim1, ulCounterValue);
      ...
      /* Something other than the tick interrupt ended the sleep.  How
       ...
       sleeping? */
      ulCompleteTickPeriods = ((uint32_t) lptim1CNT)
          / ulPeriodValueForOneTick;

      /* Check for over/under flows before the following calculation. */
      configASSERT(
          ((uint32_t ) lptim1CNT) >= (ulCompleteTickPeriods * ulPeriodValueForOneTick));
