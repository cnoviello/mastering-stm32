...
#define APP_START_ADDRESS   0x08003000 /* In STM32L476RG this corresponds with the start
                                          address of Page 5 */
#define APP_START_PAGE      6

#define SRAM_SIZE           96*1024     // STM32L476RG has 96KB of RAM
#define SRAM_END            (SRAM_BASE + SRAM_SIZE)
#define FLASH_TOTAL_PAGES   511

#define ENABLE_BOOTLOADER_PROTECTION 0
#define FIRST_PAGE_TO_PROTECT 0
#define LAST_PAGE_TO_PROTECT 5
  ...
  if (ulCrc == HAL_CRC_Calculate(&hcrc, pulData, 5) && ulSaddr >= APP_START_ADDRESS) {
    ...
    if (ulCrc == HAL_CRC_Calculate(&hcrc, (uint32_t*) pucData, 4)) {
      ...
      for (uint8_t i = 0; i < 2; i++) {
        ...
        HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, ulSaddr, ((uint64_t*)pucData)[i]);
        ulSaddr += 8;
      }
      HAL_FLASH_Lock(); //Locks again the flash memory
