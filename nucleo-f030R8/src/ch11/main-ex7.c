...
int main(void) {
  ...
  HAL_TIM_IC_Start_DMA(&htim3, TIM_CHANNEL_1, (uint32_t*)data, 2);
...
void MX_TIM6_Start(void) {
  static uint8_t odrVals[] = {0x0, 0xFF};

  htim6.Instance = TIM6;
  htim6.Init.Prescaler = 47999; //48MHz/48000 = 1000Hz
  htim6.Init.Period = 499;        //1000Hz / 2 = 500Hz
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim) {
  if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1) {
      ...
      if (data[1] >= data[0])
        ...
        uwDiffCapture = (htim3.Instance->ARR - data[0]) + data[1];

      uwFrequency = HAL_RCC_GetPCLK1Freq()/(htim3.Instance->PSC+1);
